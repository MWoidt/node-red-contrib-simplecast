...
module.exports = function (RED)
{
	...
	function SimpleCast (config)
	{
		...
		{
			node.debug("in global handler 'onError'");
			//node.client.close();
        ...
        this.onStatus = function(error, status) {
			node.debug("in status handler 'onStatus'");
		...
		this.on ('input', function (msg,send,done)
		{
			...
			{
				if(msg.payload.match("MUTE|CLOSE|UNMUTE|GET_STATUS|VOL_INC|VOL_DEC|GET_VOLUME|STOP|STATUS|PAUSE")) msg.payload = { type: msg.payload};
				else if(msg.payload == "NEXT") msg.payload = {type:"QUEUE_UPDATE", "jump":1 };
				else if(msg.payload == "PREV") msg.payload = {type:"QUEUE_UPDATE", "jump":-1 };
				else if(msg.payload == "RWD") msg.payload = {type:"SEEK_DELTA",time:-10};
				else if(msg.payload == "FWD") msg.payload = {type:"SEEK_DELTA",time:10};
				else if (msg.payload == "FRANCEINFO") msg.payload = {type:"MEDIA",media:{url:"http://direct.franceinfo.fr/live/franceinfo-midfi.mp3"}};
				else if (node.getContentType(msg.payload) != "unknow")
				{
					msg.payload = { type: "MEDIA", media: {url:msg.payload}};
				}
			}
		...
		this.clientConnect = function()
		{
			...
			try 
			{
				...
				node.debug("client created");

				// Execute command
				let connectOptions = { host: node.host };
				node.debug("connect option set up");
			...
			{
				node.onError(exception.message);
				this.status({ fill: "red", shape: "dot", text: "cant connected" });
			}
			return;
		}
        ...
        this.sendMediaCommand = function(receiver, command) {
			node.debug("in sendMediaCommand");
            // Check for load commands
			if (command.type === "MEDIA") 
			{
				node.debug("in sendMediaCommand, case MEDIA");
                // Load or queue media command
                if (command.media) {
                    if (Array.isArray(command.media)) {
                        // Queue handling
						let mediaOptions = command.media.options || { startIndex: 0, repeatMode: "REPEAT_OFF" };
			...
			{
				node.debug("in sendMediaCommand, case TTS");
                // Text to speech
                if (command.text) {
			...
			{
				node.debug("in sendMediaCommand, case media control");
                ...
                receiver.getStatus((statusError, status) => {
                    ...
                    switch (command.type) {
						...
						case "QUEUE_UPDATE":
								receiver.queueUpdate(null,{jump: command.jump},node.onStatus);
								return receiver.getStatus(node.onStatus);
								break;
							
                        case "QUEUE_NEXT":
							//receiver.next(node.onStatus);
							receiver.queueUpdate(null,{jump: 1},node.onStatus);
							return receiver.getStatus(node.onStatus);
							break;
		...
		this.sendCastCommand = function(receiver, command) 
		{
            ...
            switch (command.type) {
                ...
                case "MUTE":
                    return node.client.setVolume({ muted: true }, node.onStatus);
					break;
					
                case "UNMUTE":
                    return node.client.setVolume({ muted: false }, node.onStatus);
					break;
					
                case "VOLUME":
					if (command.volume && command.volume >= 0 && command.volume <= 100) 
					{
						return node.client.setVolume({ level: command.volume / 100 }, node.onStatus);
                    }
					break;
				...
				case "VOL_INC":
					...
					node.debug("current vol = " + v);
					if (command.step) 
					{
						v = Math.min(1,v+(command.step / 100));
						node.debug("new vol = " + v);
						return node.client.setVolume({ level: v}, node.onStatus);
					}
					else
					{
						v = Math.min(1,v+0.1);
                ...
                default:
					...
					node.debug("in sendCastCommand: case media command");	
